<div  style="height:100%;margin-bottom:0px">
  <p class="panel-heading t-panel-header" style="padding:5px;margin: 0px auto;  font-size:18px; border-bottom: 4px solid orange; width:98%">Địa điểm đề xuất</p>
  <div id="myCarousel1" class="carousel slide" data-ride="carousel" style="width:100%;padding-top: 30px; height:86%">
      <!-- Indicators -->
    <ol class="carousel-indicators" style="top:100%; ">
      <li data-target="#myCarousel1" data-slide-to="0" class="active" style="background-color: orange"></li>
      <li data-target="#myCarousel1" data-slide-to="1" style="background-color: orange"></li>
      <li data-target="#myCarousel1" data-slide-to="2" style="background-color: orange"></li>
    </ol>

    <!-- Wrapper for slides -->
    <div class="carousel-inner" style="height:99%">
      <!--Slider1-->
        <div class="item active" style="height:100%">

          <a class="t-slider-diadiemdexuat" style="height:100%" href="<?php echo base_url('dangKy'); ?>">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/banhxeo.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

          <a class="t-slider-diadiemdexuat" style="height:100%" href="<?php echo base_url('dangKy'); ?>">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/dauhu.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

          <a class="t-slider-diadiemdexuat" style="height:100%" href="#">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/comtron.png" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

        </div>
        <!--/Slider1-->

         <!--Slider2-->
        <div class="item " style="height:100%">

          <a class="t-slider-diadiemdexuat" style="height:100%" href="#">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/comrang.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

          <a class="t-slider-diadiemdexuat" style="height:100%" href="#">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/bunoc.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

          <a class="t-slider-diadiemdexuat" style="height:100%" href="#">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/bungio.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

        </div>
        <!--/Slider2-->

         <!--Slider3-->
        <div class="item" style="height:100%">

          <a class="t-slider-diadiemdexuat" style="height:100%" href="#">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/shushi.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

          <a class="t-slider-diadiemdexuat" style="height:100%" href="#">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/ga.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

          <a class="t-slider-diadiemdexuat" style="height:100%" href="#">
            <div style="height:60%">
              <img src="<?php echo base_url(); ?>/assets/images/db/bonam.jpg" alt="Địa điểm đề xuất" style="height:100%; width:100%" >
            </div>
            <div style="height:40%; background-color:white" class="t-catchuoi">
              <div class="t-diadiem-info-panel-left t-catchuoi" style="padding:10px; display: table-cell;height:100%; width:98%">
                <div class="t-catchuoi" style="font-weight: bold; font-size:18px">Quán Thằn Lằn Chiên</div>
                <div class="t-catchuoi"><em>Chuyên bán các món thằn lằn chiên, xào, đậm đà hương vị thằn lằn =]]]</em></div>
                <div class="t-catchuoi"><span class="glyphicon glyphicon-home"></span> 103 Nguyễn Du, Quận 1, TP. HCM</div>
		          </div>
            </div>
          </a>

        </div>
        <!--/Slider3-->
    </div>
      
    <!--Wrapper-->
  
  </div>
</div>